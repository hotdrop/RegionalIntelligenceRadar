...
地域シグナル / 公共政策 / 地域イノベーション

程度の小さなテキストを表示してください。

## 表示言語

日本人向けのアプリとして、見出し、操作、状態、カテゴリ、重要度、関連トピックなどのユーザー向け表示は自然な日本語を基本とします。

データ交換や型で使う英語の内部値は維持し、表示境界で日本語へ変換してください。`RIR`や`MAP_01`のような短いブランド表記・識別コードは、コンソールらしさを損なわない範囲で残して構いません。

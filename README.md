...
モバイル（950px以下）は地図→一覧→詳細の縦並びです。表示配分の「探索／均等／詳細」で地図・一覧の高さが480/480px、400/400px、320/280pxに切り替わります。詳細本文はページスクロールで読み、一覧でシグナルを選ぶと詳細へ移動します。動きを抑える設定では即時移動します。

「地域選択を解除」は倍率操作の隣にあり、倍率・位置を保ったまま全国の一覧へ戻します。LATEST REPORTは最新週、SIGNALSは全期間の総件数です。
